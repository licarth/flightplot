...
type Props = { airspaces: (Airspace | DangerZone)[]; route: OldRoute };

const toArrayCoords = (latLng: LatLng | { lat: Latitude; lng: Longitude }): [number, number] => [
    Number(latLng.lng),
    Number(latLng.lat),
];
    ...
    const overlaps: AirspaceSegmentOverlap[] = airspaces
        ...
        .flatMap(({ airspacePolygon, airspace }) => {
            const segments = route.legs
                .map(({ departureWaypoint, arrivalWaypoint, startingPointInNm, distanceInNm }) => {
                ...
                .reduce((prev, curr) => {
                    ...
                    return curr.length > 0 &&
                        ...
                        prev.length > 0 &&
                        lastPrev[1] === curr[0][0]
                        ? [...prev.slice(0, -1), [lastPrev[0], curr[0][1]], ...curr.slice(1)]
                        : [...prev, ...curr];
                });
