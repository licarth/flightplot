...
export class OldRoute {
    ...
    static factory = ({
        ...
        return new OldRoute({ lastChangeAt, waypoints, id, title, printAreas });
    };
    ...
    removeWaypoint(waypointPostion: number): OldRoute {
    ...
    get arrival() {
        return this.waypoints.length > 1 ? this.waypoints[this.waypoints.length - 1] : undefined;
    }

    get totalDistance() {
        return this.waypoints.length > 1 ? this.legs.reduce((p, c) => p + c.distanceInNm, 0) : 0;
    }
    ...
    get legs() {
        ...
        for (let i = 0; i < this.waypoints.length; i++) {
            if (this.length > i) {
                ...
                const line = [toPoint(departureWaypoint.latLng), toPoint(arrivalWaypoint.latLng)];
    ...
    verticalProfile({ aircraft }: { aircraft: Aircraft }): VerticalProfile {
        ...
        for (let i = 0; i < this.legs.length; i++) {
            const { departureWaypoint, arrivalWaypoint, distanceInNm, startingPointInNm } =
                this.legs[i];
    ...
    splitOnTouchAndGos(): OldRoute[] {
        const routes: OldRoute[] = [];
        let route = OldRoute.empty();
        for (const waypoint of this.waypoints) {
            if (
                ...
                route = OldRoute.empty().addWaypoint({ waypoint });
            } else {
    ...
    static codec = (airacData: AiracData) =>
        pipe(OldRoute.propsCodec(airacData), Codec.compose(fromClassCodec(OldRoute)));
...
export const toPoint = (latLng: LatLng): [number, number] => [latLng.lng, latLng.lat];
...
type RouteProps = Codec.TypeOf<ReturnType<typeof OldRoute.propsCodec>>;
