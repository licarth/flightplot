...
import { Marker as LeafletMarker, Point } from "leaflet";
...
const bottomLeftIcon = icon(
  "arrows-rotate",
  new Point(allIconsOffset + 45, 15)
);
const deleteIcon = icon("trash", new Point(allIconsOffset + 75, 15));
const moveIcon = icon(
  "arrows-up-down-left-right",
  new Point(allIconsOffset + 15, 15)
);
...
const SinglePrintAreaPreview = ({
  ...
  const pageWidthInLambert =
    (hundredMilliInLambert * pageFormat.dxMillimeters) / 100;
  ...
  const arrayCoords = _.mapValues(lambert, (v) =>
    toArrayCoords(convertToCoords(v))
  );
        ...
        <>
          <Marker
            ...
            eventHandlers={{
              ...
              dragend: (e) =>
                markerRef.current &&
                setBottomLeft(markerRef.current?.getLatLng()),
            }}
          />{" "}
          {!bottomLeftDrag && (
            <>
                    ...
                    setPageFormat(
                      pageFormats[
                        (pageFormats.findIndex(
                          (v) => v.dxMillimeters === pageFormat.dxMillimeters
                        ) +
                          1) %
                          pageFormats.length
                      ]
