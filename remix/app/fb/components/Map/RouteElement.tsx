...
export const RouteElement = ({
    ...
    const { route, setWaypointAltitude, setAerodromeWaypointType, replaceWaypoint } =
        routeContextWithRoute;
...
const NameDisplay = ({
    ...
    if (editingName) {
        ...
        return w.name ? (
            <NameContainer onClick={() => setEditingName(true)}>{w.name}</NameContainer>
        ) : (
        ...
        return (
            <StyledAltitudeInput
                ...
                onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                    setWaypointAltitude({
                        waypointPosition: i,
                        altitude: e.currentTarget.value ? Number(e.currentTarget.value) : null,
                    })
                }
            />
        );
