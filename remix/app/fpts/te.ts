...
import type { Lazy } from 'fp-ts/lib/function.js';
...
export namespace te {
    ...
    ) => <R1, E1>(fa: TE.TaskEither<E1, A>) => TE.TaskEither<E1 | E2, A & B> = (fb) => (fa) =>
        //@ts-ignore
        pipe(
            fa,
            //@ts-ignore
    ...
    ) => <R1, E1>(fa: TE.TaskEither<E1, A>) => TE.TaskEither<E1 | E2, A & B> = (f) => (fa) =>
        //@ts-ignore
        pipe(
            fa,
            //@ts-ignore
